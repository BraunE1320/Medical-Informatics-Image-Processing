...
function [Points] = SegmentImage1(Image)
...
while (bright(top,256) == 0)
    top = top + 1;
end
while (bright(bottom,256) == 0)
    bottom = bottom - 1;
end
while (bright(256,left) == 0)
    left = left + 1;
end
while (bright(256,right) == 0)
    right = right - 1;
end
image(bright);
hold on;
...
for angle = 0:0.01:2*pi
    ...
    point = floor(P1 + length * dirVec);
        while ((bright(point(1),point(2)) ~= 0 || bright(point(1),point(2)-1) ~= 0) && point(1) > 2 && point(2) > 2)
            length = length + 1;
            point = floor(P1 + length * dirVec)
        end
       Points(i,:) = point;
